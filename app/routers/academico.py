...
from app.models import Carrera, Materia, Estudiante, Inscripcion
...
def listar_inscripciones(materia_id: int = None, semestre: str = None, db: Session = Depends(get_db)):
...
def inscribir(data: InscripcionCreate, db: Session = Depends(get_db)):
    ...
    if existe: raise HTTPException(400, "Estudiante ya inscrito en esta materia")
